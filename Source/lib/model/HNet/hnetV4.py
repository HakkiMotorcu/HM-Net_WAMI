...
class HNet_V4 (nn.Module):
    def __init__(self, in_channels=[1,3,3],features=32,p=[0.01,0.01,0.01,0.01]):
        ...
        self.encoderA2 = self._block(features, features , name="encA2")
        self.poolA2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoderA3 = self._block(features , features , name="encA3")
        self.poolA3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoderA4 = self._block(features , features , name="encA4")
        ...
        self.encoderB2 = self._block(features, features , name="encB2")
        self.poolB2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoderB3 = self._block(features , features , name="encB3")
        self.poolB3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoderB4 = self._block(features , features , name="encB4")
        ...
        self.encoderC2 = self._block(features, features , name="encC2")
        self.poolC2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoderC3 = self._block(features , features , name="encC3")
        self.poolC3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoderC4 = self._block(features , features , name="encC4")
        self.poolC4 = nn.MaxPool2d(kernel_size=2, stride=2)


        self.bottleneck = self._block(features , features , name="bottleneck")

        #decoder heat_map
        self.upconv4 = nn.ConvTranspose2d(features , features , kernel_size=2, stride=2)
        self.decoder4 = self._block((features ) * 2, features , name="dec4")
        self.upconv3 = nn.ConvTranspose2d(features , features , kernel_size=2, stride=2)
        self.decoder3 = self._block((features ) * 2, features , name="dec3")
        ...
        self.encoderD2 = self._block(features, features , name="encD2")
        self.poolD2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoderD3 = self._block(features , features , name="encD3")
        self.poolD3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoderD4 = self._block(features , features , name="encD4")
        self.poolD4 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.bottleneckD = self._block(features , features , name="bottleneckD")
        #decoder heat_map
        self.upconvD4 = nn.ConvTranspose2d(features , features , kernel_size=2, stride=2)
        self.decoderD4 = self._block((features ) * 2, features , name="decD4")
        self.upconvD3 = nn.ConvTranspose2d(features  , features , kernel_size=2, stride=2)
        self.decoderD3 = self._block((features  ) * 2, features , name="decoderD3")
        self.upconvD2 = nn.ConvTranspose2d(features  , features , kernel_size=2, stride=2)
        self.decoderD2 = self._block((features  ) * 2, features , name="decD2")
        self.upconvD1 = nn.ConvTranspose2d(features  , features, kernel_size=2, stride=2)
    ...
    def forward(self, pre_hm,pre_img,img):
        ...
        encA11 = self.poolA1(encA1)
        encA2 = self.encoderA2(encA11)
        encA2 = encA2 + encA11
        encA21 = self.poolA2(encA2)
        encA3 = self.encoderA3(encA21)
        encA3 = encA3 + encA21
        encA31 = self.poolA3(encA3)
        encA4 = self.encoderA4(encA31)
        encA4 = encA4 + encA31
        encA41 = self.poolA3(encA4)

        encB1 = self.encoderB1(pre_img)
        encB11 = self.poolB1(encB1)
        encB2 = self.encoderB2(encB11)
        encB2 = encB2 + encB11
        encB21 = self.poolB2(encB2)
        encB3 = self.encoderB3(encB21)
        encB3 = encB3 + encB21
        encB31 = self.poolB3(encB3)
        encB4 = self.encoderB4(encB31)
        encB4 = encB4 + encB31
        encB41 = self.poolB3(encB4)

        encC1 = self.encoderC1(img)
        encC11 = self.poolC1(encC1)
        encC2 = self.encoderC2(encC11)
        encC2 = encC2 + encC11
        encC21 = self.poolC2(encC2)
        encC3 = self.encoderC3(encC21)
        encC3 = encC3 + encC21
        encC31 = self.poolC3(encC3)
        encC4 = self.encoderC4(encC31)
        encC4 = encC4 + encC31
        encC41 = self.poolC3(encA4)

        bottleneck = self.bottleneck(encA41+encB41+encC41)
        ...
        encD1 = encD1 + dec1
        encD11 = self.poolD1(encD1)
        encD2 = self.encoderD2(encD11)
        encD2 = encD2 + encD11
        encD21 = self.poolD2(encD2)
        encD3 = self.encoderD3(encD21)
        encD3 = encD3 + encD21
        encD31 = self.poolD3(encD3)
        encD4 = self.encoderD4(encD31)
        encD4 = encD4 + encD31
        encD41 = self.poolD3(encA4)
