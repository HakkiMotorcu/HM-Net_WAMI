...
#From CenterTrack
class TrackerCore(object):
  def __init__(self,setup,new_thresh=0.29,max_age=5,public_det=False,hungarian=True):
      self.new_thresh=setup.det_thres
      self.public_det=setup.is_public
      self.hungarian=setup.is_hungarian
      self.max_age=setup.max_age
      self.reset()
  ...
  def step(self, results, public_det=None):
      ...
      track_size = np.array([(track['bbox'][2] *track['bbox'][3]) for track in self.tracks], np.float32) # M
      track_cat = np.array([track['class'] for track in self.tracks], np.int32) # M
      item_size = np.array([(item['bbox'][2] * item['bbox'][3]) for item in results], np.float32) # N
      ...
      invalid = ((dist > track_size.reshape(1, M)) + (dist > item_size.reshape(N, 1)) + (item_cat.reshape(N, 1) != track_cat.reshape(1, M))) > 0
      ...
      if self.hungarian:
          ...
          # print(matched_indices,type(matched_indices))
      ...
      if self.public_det and len(unmatched_dets) > 0:
      ...
      for i in unmatched_tracks:
          ...
          if track['age'] < self.max_age:
              ...
              bbox[2] , bbox[3]])
              track['ct'] = np.array([ct[0] + v[0], ct[1] + v[1]])
              ret.append(track)
      self.tracks = ret
